...
class HFSearcher:
    def __init__(self, index=None, checkpoint=None, collection=None, config=None, index_root=None, verbose:int = 3):
        self.verbose = verbose
        if self.verbose > 1:
            print_memory_stats()
        ...
        default_index_root = initial_config.index_root_
        index_root = index_root if index_root else default_index_root
        self.index = config.index_path or os.path.join(index_root, index)
        self.index_config = ColBERTConfig.load_from_index(self.index)
        ...
        self.checkpoint = HFCheckpoint(colbert_config=self.config, verbose=self.verbose)
        
        # TODO: Create a ColBERT instance to ensure torch extensions are loaded
        self._colbert_for_extensions = ColBERT(name='bert-base-uncased', colbert_config=self.config)
        
        # Configure after checkpoint is created
    ...
    def search_emb_input(self, Q, k=1, filter_fn=None, full_length_search=False, pids=None):
        return self.dense_search(Q, k, filter_fn=filter_fn, pids=pids)
    ...
    def _search_all_Q(self, queries, Q, k, filter_fn=None, qid_to_pids=None):
        ...
        provenance = Provenance()
        provenance.source = 'HFSearcher::search_all'
        provenance.queries = queries.provenance()
        provenance.config = self.config.export()
        provenance.k = k
